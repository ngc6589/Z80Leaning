...
;       +-----+-----------------+-----+
;
;       TASM.EXE -80 -fff fileName.asm fileName.hex
;
;       2021/7/24 Masahiro Kusunoki (JP3SRS) 
;
MSTACK	.EQU	8300H
CTC0	.EQU	10H
CTC1	.EQU	11H
...
SIOATXBUFSP	.WORD	1	;SIOATXBUF STORE POINTER
SIOATXBUFGP	.WORD	1	;SIOATXBUF GET POINTER
SIOATXBUFCNT	.BYTE	1	;SIOATXBUF DATA COUNT
;
;-----------------------------------------------------------------------------
;	MODE 0 Interrupt address
;	RSTnn(nn=00H,08H,10H,18H,20H,28H,30H,38H)
	.ORG	0000H
RST00:	JP	COLDSTART
;
;-----------------------------------------------------------------------------
;	send character A REG
;
	.ORG	0008H
RST08:	JP	SIOA_PUTC
;
...
;
;-----------------------------------------------------------------------------
;	SIO2 STATUS AFFECTS ROUTINE
;
;	0: SIO CH B Transmit buffer empty
;	1: SIO CH B Change of external/status
;	2: SIO CH B Received charactor available
;	3: SIO CH B Special receive condition
;	4: SIO CH A Transmit buffer empty
;	5: SIO CH A Change of external/status
;	6: SIO CH A Received charactor available
;	7: SIO CH A Special receive condition
...
;
;-----------------------------------------------------------------------------
;	SIO CH B Transmit buffer empty
SIOI00:	HALT
	EI
	RETI
;-----------------------------------------------------------------------------
;	SIO CH B Change of external/status
SIOI01:	HALT
	EI
	RETI
;-----------------------------------------------------------------------------
;	SIO CH B Received charactor available
SIOI02:	HALT
	EI
	RETI
;-----------------------------------------------------------------------------
;	SIO CH B Special receive condition
SIOI03:	HALT
	EI
	RETI
;
;-----------------------------------------------------------------------------
;	SIO CH A Transmit buffer empty
SIOI04:
	PUSH	AF
	PUSH	HL
...
SIOI04_90:
	...
	RETI
;-----------------------------------------------------------------------------
;	SIO CH A Change of external/status
SIOI05:	HALT
	EI
	RETI
;-----------------------------------------------------------------------------
;	SIO CH A Received charactor available
SIOI06:
	PUSH	AF
	PUSH	HL
SIOI06_10:
...
SIOI06_90:
	...
	RETI
;-----------------------------------------------------------------------------
;	SIO CH A Special receive condition
SIOI07:	HALT
	EI
	RETI
;
...
SIOA_PUTC_10:
	LD	A,D
        LD      HL,(SIOATXBUFSP)        ;STORE CHARACTER DATA
        LD      (HL),A
...
;-----------------------------------------------------------------------------
;	SIO A COMMAND CHAIN
SIOACC:	.BYTE	00 011 000B	;WR0 CHANNEL RESET
	.BYTE	00 000 001B	;WR0 -> WR1
	.BYTE	000 11 0 1 0B	;WR1 RX INTERRUPT
                                ;    TX INTERRUPT
	.BYTE	00 000 011B	;WR0 -> WR3
...
;				;    non parity
	...
	.BYTE	1 11 0 1 0 1 0B	;WR5 DTR=ON
                                ...
                                ;    ENABLE TRANSMITTER
                                ;    RTS=ON
SIOACE	.EQU	$		;END OF COMMAND CHAIN

SIOBCC:	.BYTE	00 011 000B	;WR0 CHANNEL RESET
	.BYTE	00 000 001B	;WR0 -> WR1
	.BYTE	000 00 1 0 0B	;WR1 ENABLE STATUS AFFECTS VECTOR
	.BYTE	00 000 010B	;WR0 -> WR2
	.BYTE	SIO2VECT-RST00	;WR2 INTERRUPT VECTOR
SIOBCE	.EQU	$
;
;-----------------------------------------------------------------------------
;       SIOA RESET RXBUFFER VARIABLES
SIOA_INITRXVAR:
        LD      A,0
        LD      (SIOARXBUFCNT),A
        LD      HL,SIOARXBUF
...
;	MAIN ROUTINE
LOOP:
        JP      COLD                    ;JUMP TO BASIC COLD ROUTINE
;
;
;==================================================================================
...
;
MEMMSG: .BYTE	"Memory size not enough",CR,LF
	.BYTE	"The system is stopped.",CR,LF,0,0
;
